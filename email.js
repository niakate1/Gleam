...
const templates = {
  ...
  // 3. Devis accepté → Pro
  devis_accepte: (d) => ({
    // Le corps disait déjà « Le paiement du client est en attente », mais le
    // titre promettait le contraire — et c'est le titre qu'on lit dans sa
    // boîte, souvent sans ouvrir.
    subject: `Votre devis a été retenu`,
    html: wrapTemplate({
      title: `Bonjour ${d.prenom},`,
      // ── L'AVERTISSEMENT PASSE AVANT LE CRÉNEAU ────────────────────────
      // Il venait après. Or c'est le créneau qu'on retient d'un courriel — on
      // le note, on bloque sa journée, on décline autre chose. L'avertissement
      // arrivait trop tard dans la lecture.
      //
      // Le créneau est désormais annoncé comme PRÉVU, pas comme confirmé.
      body: `<p>Le client a choisi votre devis pour <strong>${d.prestation}</strong>.</p>
             <p style="background:#FFF8E9;border-left:3px solid #E0B44A;padding:10px 12px;margin:14px 0">
               <strong>Ne vous déplacez pas encore.</strong><br>
               Le client n'a pas encore réglé : la prestation n'est pas confirmée.
...
async function sendEmail(type, to, data = {}) {
  // ── UN COURRIEL NE DOIT JAMAIS INTERROMPRE UN FLUX ──────────────────────
  // Cette fonction levait une exception sur un type inconnu. Elle est appelée
  // depuis l'envoi de devis, la validation de prestation, le paiement — des
  // opérations où l'argent et l'engagement sont en jeu.
  //
  // Une faute de frappe dans un nom de gabarit faisait donc échouer un devis
  // entier, avec un message incompréhensible pour le prestataire.
  //
  // On journalise et on renonce à l'envoi : le flux principal continue.
  const builder = templates[type];
  if (!builder) {
    console.error(`[email] Type inconnu : "${type}" — courriel non envoyé. `
      + `Types valides : ${Object.keys(templates).join(', ')}`);
    return;
  }

  // Un gabarit peut échouer sur une donnée absente — un prénom, un montant.
